...
path_to_old_test = r"\\babyserverdw5\Digital pathology image lib\_Image libraries for training\2023-05-09 Published HE Nuclei Datasets\old\20x split_old\test"
extensions = ['.TIFF', '.tif']
...
def score_scenario(scenario: pd.Series, all_data: dict, old_tst: dict, models_folder: str,
    ...
    for i, val in scenario.items():
        if i == 'Model Name':
            ...
            if model_name == 'Model_00':
                model = sd.load_model(model_path=os.path.join(models_folder, model_name))
            elif model_name == 'Published_40x':
                model = sd.load_published_he_model(folder_to_write_new_model_folder=models_folder,
                                                   name_for_new_model=model_name)
        elif i == 'Percentile Splits':
            if model_name == 'Published_40x':
                splits = eval(val)
...
def read_old_test(path_to_old_tst: str) -> dict:
    tst = {}
    for dataset in os.listdir(path_to_old_tst):
        tst[dataset] = {'Names': [], 'Images': [], 'Annotations': []}
        path_dataset = os.path.join(path_to_old_tst, dataset)
        img_path = os.path.join(path_dataset, 'images')
        msk_path = os.path.join(path_dataset, 'masks')
        for tilename in os.listdir(img_path):
            if tilename.endswith('.TIFF'):
                tst[dataset]['Names'].append(tilename[:-5])
                tst[dataset]['Images'].append(imread(os.path.join(img_path, tilename)))
                tst[dataset]['Annotations'].append(imread(os.path.join(msk_path, tilename)))
    return tst
...
if __name__ == "__main__":
    ...
    old_test = read_old_test(path_to_old_test)
    ...
    for _, row in tqdm(input_df.iterrows(), desc="Progress through Models"):
        ...
        single_score = score_scenario(row, full_dataset, old_test, path_to_models,
                                      skin1_, skin2_, pancreas_, fallopian_tube_)
